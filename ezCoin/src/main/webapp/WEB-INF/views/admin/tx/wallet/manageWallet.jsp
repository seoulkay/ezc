 ...
 * 검색
 */
function getSearchWalletList() {
	$('#searchForm').attr("action", "/admin/tx/wallet/searchWalletPro.do");
	$('#searchForm').submit();
}
...
<body>
	...
	<jsp:include page="../../common/navbar.jsp" />
			...
			<ol class="breadcrumb pull-right" style="font-size: 12px;">
				  ...
				  <li>거래 관리</li>
				  <li><a href="/admin/tx/manageWallet.do">전자 지갑 관리</a></li>
			</ol>
          <h4 class="sub-header"><span class="glyphicon glyphicon-asterisk"></span> <b>전자 지갑 관리</b></h4>
          </div>
         </div>
       		<c:set var="boardType" value="Wallet"/>
   <div class="row">
            ...
            <col width="10%"/>
              ...
              <thead>       
                <tr>
                  ...
                  <th class="text-center">Bitcoin 입금 주소</th>
                  <th class="text-center">Bitcoin 출금 주소</th>
              ...
              <tbody>
                    ...
                    <c:if test="${fn:length(vo) > 0}">                		
						...
						<c:forEach var="vo" items="${vo}" varStatus="status">
			                 <tr>
			                     <td><c:out value="${vo.realbtcIdx }" /></td>
			                     <td><c:out value="${vo.realbtcType }" /></td>
			                     <td><a href="/admin/member/info/updateMember.do?memberIdx=${vo.memberIdx }" target="_blank"><c:out value="${vo.memberId }" /></a></td>
			                     <td><c:out value="${vo.realbtcAddressIn }" /></td>
			                     <td><c:out value="${vo.realbtcAddressOut }" /></td>
			                     <td><c:out value="${vo.realbtcBtc }" /></td>
			                     <td>
			                     <fmt:formatDate value="${vo.realbtcDatetime }"  type="both" pattern="yyyy-MM-dd"/>
			                     </td>
