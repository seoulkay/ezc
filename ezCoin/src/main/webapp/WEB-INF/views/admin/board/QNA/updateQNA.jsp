...
function insertQNABoard() {
	...
	$('#form').attr("action","/admin/board/QNA/updateQNAPro.do");
	$('#form').submit();
}
...
<body>
	...
	<jsp:include page="../../common/navbar.jsp" />
			...
			<ol class="breadcrumb pull-right" style="font-size: 12px;">
				  ...
				  <li>ezCoin에 바란다 수정</li>
			</ol>
          <h4 class="sub-header"><span class="glyphicon glyphicon-asterisk"></span> <b>QNA 수정</b></h4>
          </div>
       
  <form id="form" name="form" method="post">
     <input type="hidden" id="useIs" name="useIs"/>
     <input type="hidden" id="askMngIdx" name="askMngIdx" value="${vo.askMngIdx}"/>  
     ...
     <input type="hidden" id="createDate" name="hitcount" value="${vo.createDate}"/>  
          ...
          <div class="table-responsive">
						<table class="table table-striped">
							...
							<tr>
								<th>제목</th>
								<td><input type="text" name="subject" title="제목" style="width:100%" maxlength="256" id="subject" value="${vo.subject }" readonly/></td>
							</tr>
							<tr>
								<th>질문자ID</th>
								<td>${vo.createId }</td>
							...
							<tr>
								<th>질의시간</th>
								<td>
								<fmt:formatDate value="${vo.createDate }"  type="both" pattern="yyyy-MM-dd hh:mm"/> 
								</td>
							</tr>

							<tr>
								<th>내용</th>
								<td><textarea name="content" rows="6" style="width:100%" id="content" onkeyup="javascript:checkTextAreaMax(this)" readonly>${vo.content }</textarea></td>
							</tr>
							<tr>
								<th>사용여부</th>
								<td>
							...
							<tr>
								<th>답변</th>
								<td><textarea name="response" rows="6" style="width:100%" id="response" onkeyup="javascript:checkTextAreaMax(this)">${vo.response }</textarea></td>
							</tr>
