...
function getSearchList() {
	$('#searchForm').attr("action", "/admin/tx/KRW/searchKRWOutputPro.do");
	$('#searchForm').submit();
}
...
<body>
	...
	<jsp:include page="../../common/navbar.jsp" />
			...
			<ol class="breadcrumb pull-right" style="font-size: 12px;">
				  ...
				  <li>거래 관리</li>
				  <li><a href="/admin/tx/manageKRWOutput.do">원화 출금 관리</a></li>
			</ol>
          <h4 class="sub-header"><span class="glyphicon glyphicon-asterisk"></span> <b>원화 출금 관리</b></h4>
          </div>
         </div>
          ...
          <div class="table-responsive">
            ...
            <col width="10%"/>
              ...
              <tbody>
              		...
              		<tr>
              			<td colspan="9" style="text-align: center">거래가 없습니다.</td>
              		</tr>
              		</c:if>
                    <c:if test="${fn:length(vo) > 0}">                		
						...
						<c:forEach var="vo" items="${vo}" varStatus="status">
			                 <tr>
			                     <td><c:out value="${vo.wonRequestIdx }" /></td>
			                     <td><a href="/admin/member/info/updateMember.do?memberIdx=${vo.memberIdx }" target="_blank"><c:out value="${vo.memberId }" /></a></td>
			                     <td>
			                     <fmt:formatDate value="${vo.wonRequestDatetime }"  type="both" pattern="yyyy-MM-dd hh:mm"/>
			                     </td>
			                     <td><c:out value="${vo.memberBank }" /></td>
			                     <td><c:out value="${vo.memberBankAccount }" /></td>
			                     <td><c:out value="${vo.wonRequestKrw }" /></td>
			                     <td><c:out value="${vo.wonRequestResponseId }" /></td>
			                     <td>
			                     <fmt:formatDate value="${vo.wonRequestResponseDatetime }"  type="both" pattern="yyyy-MM-dd hh:mm"/>
			                     </td>
			                     <td><a href="" class="btn btn-danger btn-xs">확인</a></td>
