 ...
 * 검색
 */
function getSearchQNAList() {
	$('#searchForm').attr("action", "/admin/board/QNA/searchQNAPro.do");
	$('#searchForm').submit();
}
 ...
 * 수정 이동.
 */
function updateQNA(QNAMngIdx) {
	$('#QNAMngIdx').val(QNAMngIdx);
	$('#form').attr("action", "/admin/board/QNA/updateQNA.do");
	$('#form').submit();
}
...
function deleteQNA(QNA_idx) {
	if(confirm("게시물을 삭제 하시겠습니까?")) {
		$('#QNAMngIdx').val(QNA_idx);
		$('#form').attr("action", "/admin/board/QNA/deleteQNAPro.do");
		$('#form').submit();
	} else {
		alert("취소 되었습니다.");
	}
...
<body>
	...
	<jsp:include page="../../common/navbar.jsp" />
          ...
          <h4 class="sub-header"><span class="glyphicon glyphicon-asterisk"></span> <b>ezCoin에 바란다 관리</b></h4>
          </div>
          </div>
       		<c:set var="boardType" value="QNA"/>
   <div class="row">
...
<form action="#" id="searchForm" method="get" class="form-inline text-center" role="form">
	...
	<a class="btn btn-sm btn-info" onclick="javascript:getSearch${boardType}List()"><span class="glyphicon glyphicon-search"></span> 검색</a>           	    
          ...
          <input type="hidden" id="QNAMngIdx" name="QNAMngIdx"/>
...
<%--                			<c:set var="index" value="${extractBoardVO.count - ((extractBoardVO.page - 1) * 20)}"/> --%>
						<c:forEach var="QNAList" items="${QNAList}" varStatus="status">
			                 <tr>
			                     <td><c:out value="${QNAList.askMngIdx }" /></td>
			                     <td>
<%-- 			                     <a href="javascript:moveBoardDetail('${QNAList.board_idx }')"> --%>
...
<%-- 								 <c:out value="${fn:replace(extractBoardList.board_name, searchTxt, replaceTxt)}" escapeXml="false" /></a></td> --%>
								 <a href="javascript:updateQNA('${QNAList.askMngIdx }')" ><c:out value="${QNAList.subject }"/></a>
								 </td>
			                     <td>
			                     <c:if test="${QNAList.response != null}">
			                     ...
			                     <td>
			                     <fmt:formatDate value="${QNAList.createDate }"  type="both" pattern="yyyy-MM-dd"/>
			                     </td>
			                     <td>
			                     <fmt:formatDate value="${QNAList.responseCreateDate }"  type="both" pattern="yyyy-MM-dd"/>
			                     </td>
			                     <td style="text-align: center">
			                     <c:if test="${QNAList.useIs == 1}">
			                     <span class="glyphicon glyphicon-ok-circle" style="color: green"></span>
			                     </c:if>
			                     <c:if test="${QNAList.useIs == 0}">
			                     <span class="glyphicon glyphicon-ban-circle" style="color: red"></span>
			                     </c:if>
			                     </td>
			                     <td><a href="javascript:deleteQNA('${QNAList.askMngIdx }')" class="btn btn-warning btn-xs">삭제</a>
			                     </td>
